...
* **Nodes**: Each node in the tree represents **a state of the retrosynthesis problem**. In AiZynthFinder, a node corresponds to **a set of intermediate molecules** that can be used to **synthesise the target molecule**.
...
MCTS is applied in AiZynthFinder as follows:

**1. Simulation Process**
* **Random Sampling**:
  * MCTS uses **random sampling** called **Monte Carlo simulations** to explore potential synthetic routes.
  * Starting from the **root node** (target molecule), the algorithm **randomly selects and applies** one of the **reaction templates** in the **ranked list generated by the expansion policy neural network** to generate precursor molecules.
  * This process is repeated for **each resulting precursor molecule** until the the **stop criterion is met** (usually either when it reaches commercially available precursors, or it reaches a **pre-defined tree depth/number of disconnections**)
* 
